with tab as
 (select a.街道名称,
    ...
    from (select count(1) 面访质控情况面访人数,
         ...
         (select count(distinct id) 面访质控情况疾病信息不一致人数, mfryid
            from (select a.id, a.mfryid
         ...
         (select count(distinct a.id) 肿瘤疾病信息不一致人数, a.mfryid
            from grjbxx a, dljbsxx b, dlmfxx c
           where a.id = b.id(+)
             and b.ex_zt(+) = '1'
             and a.id = c.grid
         ...
         (select count(distinct a.id) 慢性肾病疾病信息不一致人数, a.mfryid
            from grjbxx a, dljbsxx b, dlmfxx c
           where a.id = b.id(+)
             and b.ex_zt(+) = '1'
             and a.id = c.grid
         ...
         (select count(distinct a.id) 脑卒中疾病信息不一致人数, a.mfryid
            from grjbxx a, dljbsxx b, dlmfxx c, dlmfxx_nzz d
           where a.id = b.id(+)
             and c.id = d.id
             and b.ex_zt(+) = '1'
         ...
         (select count(distinct a.id) 心肌梗死疾病信息不一致人数, a.mfryid
            from grjbxx a, dljbsxx b, dlmfxx c, dlmfxx_xjgs d
           where a.id = b.id(+)
             and c.id = d.id
             and b.ex_zt(+) = '1'
             ...
             <if if(1 == 1)>
           group by a.mfryid) b4,
         (select count(distinct a.id) 充血性心力衰竭不一致人数, a.mfryid
            from grjbxx a, dljbsxx b, dlmfxx c, dlmfxx_xsj d
           where a.id = b.id(+)
             and c.id = d.id
             and b.ex_zt(+) = '1'
         ...
         (select count(1) 充血性心力衰竭面访人数,
           ...
           group by a.mfryid) c4
   where a.mfryid = b.mfryid(+)
     and a.mfryid = b1.mfryid(+)
     and a.mfryid = b2.mfryid(+)
     and a.mfryid = b3.mfryid(+)
     and a.mfryid = b4.mfryid(+)
     and a.mfryid = b5.mfryid(+)
     and a.mfryid = c.mfryid(+)
     and a.mfryid = c1.mfryid(+)
     and a.mfryid = c2.mfryid(+)
     and a.mfryid = c3.mfryid(+)
     and a.mfryid = c4.mfryid(+))
select 街道名称,
       面访质控情况面访人数,
       面访质控情况疾病信息不一致人数,
       面访质控情况质控人数,
