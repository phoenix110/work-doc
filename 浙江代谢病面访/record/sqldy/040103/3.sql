with tab as
 (select a.街道名称,
         ...
         f.慢性肾病疑似数,
         f.慢性肾病非病例数
    from (select b.mc 街道名称, a.xzqh_x
            from grjbxx a, p_xzdm b, dlmfxx c
           where a.xzqh_x = b.dm
             and a.id = c.grid
   ...
   where a.xzqh_x = b.xzqh_x(+)
     ...
     and a.xzqh_x = d.xzqh_x(+)
     and a.xzqh_x = e.xzqh_x(+)
     and a.xzqh_x = f.xzqh_x(+)
     and (b.肿瘤发现病例数 >0 or c.脑卒中发现病例数 > 0 or d.心肌梗死发现病例数 > 0 or e.充血性心力衰竭发现病例数 > 0 or f.慢性肾病发现病例数 > 0))
select   街道名称,
         肿瘤发现病例数,
         肿瘤复核数,
         肿瘤确诊数,
