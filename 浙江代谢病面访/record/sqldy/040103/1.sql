with tab as
 (select a.应调查人数,
         ...
         b3.复核充血性心力衰竭,
         b.复核慢性肾病,
         (nvl(b.复核糖尿病,0) + nvl(b.复核肿瘤,0) + nvl(b1.复核脑卒中,0) + nvl(b2.复核心肌梗死,0) + nvl(b3.复核充血性心力衰竭,0) + nvl(b.复核慢性肾病,0)) 复核小计
    from (select count(1) 应调查人数,
         ...
         (select sum(decode(b.sczt, '1', 1, 0)) 存活数,
                 ...
                 sum(case when b.dia = '1' and c.id is not null then 1 else 0 end) 复核糖尿病,
