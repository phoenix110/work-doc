CREATE OR REPLACE Procedure exe_rp_bdkz_bnb(year    in VARCHAR2,
                                            month   in VARCHAR2,
    ...
    cursor r_report is
      select t.code, t.name, t.remark1
       ...
       where t.code_info_id = 123
         and t.operator like '%B%';
    r_row r_report%rowtype;
  begin
    --初始化时间值
    V_JZSJ_LS_1 := to_date(year || month || '10', 'yyyyMMdd');
    V_JZSJ_LS_2 := to_date(year || month || '15', 'yyyyMMdd');
    V_JZSJ_LS_3 := to_date(year || month || '20', 'yyyyMMdd');
    V_JZSJ_LS_4 := to_date(year || month || '25', 'yyyyMMdd');
