create or replace procedure      zjjk_shjc_xbyt_report
(
 ...
 p_codeinfo out zjjk_report.refcursor--返回是否故意数据
)
is
  begin
       open p_cur for
       ...
       group by ty.vc_sfgy order by ty.vc_sfgy;
       ...
       open p_codeinfo for
       select t.code,t.name from code_info t,code_info s where t.code_info_id=s.id  and s.code='DICT_SHJC_SFGY'
       order by t.code;

  end zjjk_shjc_xbyt_report;
