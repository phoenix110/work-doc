create or replace procedure      updateGJSJDR3
is
 ...
 where t.vc_gbswyyicd = t1.vc_gbsy group by t.vc_bgkid having count(t.vc_bgkid)=1)
 ...
 and  not exists ( select t4.vc_bgkid
     from zjjk_swbgk_temp4 t4 where t4.vc_bgkid=t.vc_bgkid);
        loop
        a0:=null;
        a1:=null;
        a2:=null;
    ...
    if (length(trim(a2)) = 2) then
      ...
      ,t.dt_lrsj=a4,t.dt_shsj=a6
             where t.vc_bgkid=a3 ;
       insert into zjjk_swbgk_temp5 (select * from zjjk_swbgk_temp1 where vc_bgkid=a5);
        commit;
      end loop;
      close p_cur1;
end updateGJSJDR3;
