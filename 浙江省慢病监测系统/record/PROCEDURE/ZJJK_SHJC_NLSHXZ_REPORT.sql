create or replace procedure      zjjk_shjc_nlshxz_report
(
 ...
 p_codeinfo out zjjk_report.refcursor
)
is
  begin
       open p_cur for
       ...
       group by ty.vc_shxz1 order by to_number(ty.vc_shxz1);
       ...
       where ty.vc_shxz1 is not null
             ...
             and to_char(ty.dt_shrq,'yyyy-MM-dd')>=to_char(to_date(beginDate,'yyyy-MM-dd'),'yyyy-MM-dd')
             and to_char(ty.dt_shrq,'yyyy-MM-dd')<=to_char(to_date(endDate,'yyyy-MM-dd'),'yyyy-MM-dd')
       order by ty.vc_shxz1;

       open p_codeinfo for
       select ty.code,ty.name from code_info ty
       where ty.code_info_id='9' or ty.code_info_id='10' or ty.code_info_id='11'
       order by ty.code;
  end zjjk_shjc_nlshxz_report;
