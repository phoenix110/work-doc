create or replace procedure      zjjk_shjc_yyshbw_report(
...
is
  begin
       open p_cur for
       select substr(ty.vc_ssbw1,0,1) ssbw,ty.vc_shyy,
              count(ty.vc_bgkid) c1
       from zjmb_shjc_bgk ty
       where ty.vc_shxz1 is not null
             and ty.vc_bgkzt='0'
             and ty.vc_shyy is not null and substr(ty.vc_ssbw1,0,1) in ('1','2','3','4','5','6','7')
       ...
       group by substr(ty.vc_ssbw1,0,1),ty.vc_shyy order by substr(ty.vc_ssbw1,0,1),ty.vc_shyy;
       ...
       open p_codeinfo for
       select t.code,t.name from code_info t,code_info s where t.code_info_id=s.id  and s.code='DICT_SHJC_SSBW'
       order by t.code;
       open p_shyy for
       select t.code,t.name from code_info t,code_info s where t.code_info_id=s.id  and s.code='DICT_SHJC_SSYY'
       order by t.code;
end zjjk_shjc_yyshbw_report;
