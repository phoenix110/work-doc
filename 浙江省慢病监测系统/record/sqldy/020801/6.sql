...
select b2.vc_bgdw,
       b2.uuid,
       b2.VC_YYRID vc_yywybs,
       b2.validate_date vc_yzsj,
       b1.vc_hzxm,
       b2.validate_detail vc_cwyy,
       count(1) over() total
 ...
 where b1.vc_personid(+) = b2.vc_hzid
   ...
   <if if(StringUtils.isNotBlank(#{dt_drsj_ks}))>
      and b2.validate_date >= std(#{dt_drsj_ks}, 1)
   </if>
   <if if(StringUtils.isNotBlank(#{dt_drsj_js}))>
      and b2.validate_date <= std(#{dt_drsj_js}, 1)
   </if>
   <if if(StringUtils.isNotBlank(#{vc_xm}))>
      and b1.vc_hzxm like #{vc_xm} || '%'
   </if>
   order by b2.validate_date desc
 ) where rownum <= #{rn_e}
 ) where rn >= #{rn_s}