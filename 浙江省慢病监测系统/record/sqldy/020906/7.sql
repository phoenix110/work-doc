with tab as
 (select dm, mc qymc, bks, bgjss, shjss, zzqss, cks
    from (select dm, mc from p_xzdm where sjid = #{vc_hks}) a,
         ...
         (select count(distinct t1.vc_bgkid) cks,
                 substr(t1.vc_cjdwdm, 1, 6) qydm
            from zjmb_shjc_bgk t1, zjmb_shjc_bgk t2
           where t1.vc_bgkid <> t2.vc_bgkid
             ...
             and t2.vc_bgkzt = '0'
             and substr(t1.vc_cjdwdm, 1, 6) = substr(t2.vc_cjdwdm, 1, 6)
             and t1.vc_cjdwdm like substr(#{vc_hks}, 1, 4) || '%'
             and t2.vc_cjdwdm like substr(#{vc_hks}, 1, 4) || '%'
             </if>
             <if if(StringUtils.isNotBlank(#{dt_jzsj_ks}))>
             ...
             group by substr(t1.vc_cjdwdm, 1, 6)
             ) c
   where substr(a.dm, 1, 6) = b.qydm(+)
     and substr(a.dm, 1, 6) = c.qydm(+))
select qymc, bks, bgjss, shjss, zzqss, cks, bgjss_l, shjss_l, zzqss_l
  from (select dm,
               qymc,
               bks, 
               bgjss, 
