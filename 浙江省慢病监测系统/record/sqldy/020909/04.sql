 ...
 (SELECT VC_CZHKQX cfsi,
    ...
    FROM (SELECT bgk.VC_CZHKQX,
           ...
           WHERE sfk.vc_bgkid = bgk.vc_bgkid
             AND bgk.vc_scbz = '2'
             AND bgk.vc_kzt IN ('0', '5', '7')
             AND bgk.vc_gxbzd IS NOT NULL AND vc_czhksi = #{vc_hks}
						 </if>
						 <if if(StringUtils.isNotBlank(#{vc_hkqx}))>
							  AND VC_CZHKQX = #{vc_hkqx}
						 </if>
...
ck AS 
(SELECT VC_CZHKQX, SUM(counts) cks
  FROM (SELECT VC_CZHKQX, COUNT(1) counts
          FROM zjjk_xnxg_bgk bgk
         WHERE bgk.vc_scbz = '2'
           AND bgk.vc_kzt IN ('0', '5', '7')
           AND bgk.vc_gxbzd IS NOT NULL AND vc_czhksi = #{vc_hks}
