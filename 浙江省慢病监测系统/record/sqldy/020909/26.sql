...
cf AS
 (SELECT VC_HKJD cfsi,
    ...
    FROM (SELECT hzxx.VC_HKJD,
           ...
           WHERE bgk.vc_hzid = hzxx.vc_personid AND sfk.vc_bgkid = bgk.vc_bgkid
             AND bgk.vc_scbz = '2'
             AND bgk.vc_bgkzt IN ('0', '5', '7')
...
ck AS 
(SELECT VC_HKJD, SUM(counts) cks
  FROM (SELECT VC_HKJD, COUNT(1) counts
          FROM zjjk_tnb_bgk bgk,zjjk_tnb_hzxx hzxx
         WHERE bgk.vc_hzid = hzxx.vc_personid AND bgk.vc_scbz = '2'
           AND bgk.vc_bgkzt IN ('0', '5', '7')
