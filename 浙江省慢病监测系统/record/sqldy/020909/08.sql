...
ck AS 
(SELECT VC_HKQXDM, SUM(counts) cks
  FROM (SELECT VC_HKQXDM, COUNT(1) counts
          FROM zjjk_zl_bgk bgk,zjjk_zl_hzxx hzxx
         WHERE bgk.vc_scbz = '2'
           AND bgk.VC_BGKZT IN ('0', '5', '7')
           AND VC_HKSDM = #{vc_hks}
