   ...
   WHERE bgk.vc_scbz = '2'
     ...
     AND bgk.vc_gxbzd IS NOT NULL
		  <if if("Y".equals(#{bblx}))>
			...
			<if if(1 == 1)>
   GROUP BY bgk.vc_czhksi),
cf AS
 (SELECT vc_czhksi cfsi,
...
ck AS 
(SELECT vc_czhksi, SUM(counts) cks
  FROM (SELECT vc_czhksi, COUNT(1) counts
          FROM zjjk_xnxg_bgk bgk
         WHERE bgk.vc_scbz = '2'
           AND bgk.vc_kzt IN ('0', '5', '7')
           AND bgk.vc_gxbzd IS NOT NULL
