  ...
  from (select vc_bgkid,
          ...
          from (SELECT a.vc_bgkid,
                 ...
                 where a.vc_hzid = b.vc_personid
                   ...
                   AND EXISTS (SELECT 1
                                 FROM p_yljgjb jg
                                WHERE a.vc_bgdw = jg.jgdm
                                  AND jg.jbgjb IN ('县级','地市级','省级'))
                   AND EXISTS (SELECT 1
	                               FROM zjjk_zlfh_ccjg ccjg
				                        WHERE ccjg.ywlx = '01'
				                          AND a.vc_bgdw = ccjg.jgdm)
                   and a.vc_scbz = '0'
                   AND EXISTS (SELECT 1
                                 FROM zjjk_zlfhsj sj
                                WHERE trunc(a.dt_cjsj) > trunc(sj.dt_ksrq)
                                  AND trunc(a.dt_cjsj) <= trunc(sj.dt_jsrq)
                                  AND sj.ccbz LIKE '%3%'
                                  AND (sj.tnbicd10 IS NULL OR sj.tnbicd10||',' LIKE '%'||SUBSTR(a.vc_icd10,0,3)||',%')
                                  AND (sj.bgkzt is null or a.vc_bgkzt in (select column_value from TABLE(split(sj.bgkzt, ','))))
                                  AND sj.zt='1')
                   AND NOT EXISTS(SELECT 1
                                    FROM zjjk_mb_zlfh fh
                                   WHERE fh.bgkid = a.vc_bgkid
																	   AND fh.mblx = '3'
